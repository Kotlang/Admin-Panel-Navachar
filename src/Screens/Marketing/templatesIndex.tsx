...
const TemplatesIndex: React.FC<props> = ({ templateSelection }) => {
                ...
                clients.messaging.messaging.FetchTemplates(
                    ...
                    (err, res) => {
                        if (err) {
                            ...
                            const fetchedTemplates: MessagingTemplate[] =
                                res.getTemplatesList();
    ...
    useEffect(() => {
        fetchTemplates()
            .then((templates) => {
                const templateData: templateCardDetails[] = templates.map(
                    (template) => {
                        return {
        ...
        <List
            ...
            renderItem={(item, index) => (
                <List.Item
                    key={index}
                    extra={
                        item.mediaUrl && <img width={272} alt="logo" src={item.mediaUrl} />
                    }
                ...
                >
                    ...
                    {templateSelection ? (
                        <Link
                            to={`/marketing/createmessage/${item.Id}`}
                            state={{
                                body: item.Content,
                                bodyParams: item.templateData.getBodyparametersMap(),
                                header: item.templateData.getHeader(),
                                headerParams: item.templateData.getHeaderparametersMap(),
                                footer: item.templateData.getFooter(),
                            }}
                        >
                            <CardDetails {...item} />
                        </Link>
